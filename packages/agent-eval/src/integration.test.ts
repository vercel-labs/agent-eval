...
const hasBubCredentials = !!process.env.BUB_API_KEY && hasSandbox;
...
`
  ...
  describe.skipIf(!hasBubCredentials)('Bub (Direct API) sandbox execution', () => {
    it('can run a simple eval with Bub', async () => {
      // Create a simple test fixture
      const fixtureDir = join(TEST_DIR, 'simple-eval-bub');
      mkdirSync(join(fixtureDir, 'src'), { recursive: true });

      writeFileSync(
        join(fixtureDir, 'PROMPT.md'),
        'Add a function called greet that returns "Hello from Bub!"'
      );
      writeFileSync(
        join(fixtureDir, 'EVAL.ts'),
        `
...
`
      ...
      const fixture = loadFixture(TEST_DIR, 'simple-eval-bub');
    ...
    it('verifies Bub result output structure matches expected format', async () => {
      // Create a simple test fixture
      const fixtureDir = join(TEST_DIR, 'result-structure-bub');
      mkdirSync(join(fixtureDir, 'src'), { recursive: true });
...
`
      ...
      const fixture = loadFixture(TEST_DIR, 'result-structure-bub');
