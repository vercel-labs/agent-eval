...
- [#201](https://github.com/vercel-labs/agent-eval/pull/201) [`2cc99ee`](https://github.com/vercel-labs/agent-eval/commit/2cc99ee6bf984d35517c53208a292fa1a8f9f2d4) Thanks [@molebox](https://github.com/molebox)! - Upgrade `@vercel/sandbox` to version 3 while preserving the existing Node.js 24 runtime and ephemeral sandbox lifecycle. Version 3 transparently starts a new session when the previous one stopped; because eval sandboxes are ephemeral that would continue a run on an empty filesystem, so the wrapper now fails the run with `SandboxSessionRecycledError` instead, matching the hard failure previous versions produced.
